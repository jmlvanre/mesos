...
void socket_send_data(int s, SendRequest* request)
{
  ...
  while (true) {
    ...
    if (length < 0 && (errno == EINTR)) {
      ...
      // Might block, try again later.
      io::poll(s, io::WRITE)
        .onAny(lambda::bind(&internal::socket_send_data, s, request));
      return;
    } else if (length <= 0) {

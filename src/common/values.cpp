...
Value::Ranges coalesce(std::vector<Range>&& ranges) {
  // Exit early if empty.
  if (ranges.empty()) {
     return Value::Ranges();
  }
  ...
  // Reserve enough space so we allocate just once.
  Value::Ranges result;
...
Value::Ranges coalesce(const Value::Ranges& lhs, const Value::Ranges& rhs) {
  std::vector<internal::Range> ranges(lhs.range_size() + rhs.range_size());

  // Merge the ranges into a vector.
  ...
  fill(lhs, 0);
  fill(rhs, lhs.range_size());

  return internal::coalesce(std::move(ranges));
}


// Coalesce the given 'ranges'.
Value::Ranges coalesce(const Value::Ranges& ranges) {
  return coalesce(ranges, Value::Ranges());
}


// Coalesce the given '_rhs' into 'lhs' ranges.
Value::Ranges coalesce(const Value::Ranges& lhs, const Value::Range& _rhs) {
  Value::Ranges rhs;
  rhs.add_range();
...
static void remove(Value::Ranges* _ranges, const Value::Range& removal)
{
  ...
  for (const Value::Range& range : _ranges->range()) {
    ...
    if (range.begin() < removal.begin() && range.end() > removal.end()) {
      ranges.emplace_back(internal::Range{range.begin(), removal.begin() - 1}); // Front.
      ranges.emplace_back(internal::Range{removal.end() + 1, range.end()}); // Back.
    }
  ...
  *_ranges = coalesce(std::move(ranges));
}
...
{
  Value::Ranges left = coalesce({_left});

  Value::Ranges right = coalesce({_right});
...
Value::Ranges operator-(const Value::Ranges& left, const Value::Ranges& right)
{
  Value::Ranges ranges = coalesce(left);
  return ranges -= right;
}
...
{
#if 0
  // TODO(joerg84): We should define the left side being sorted and
  // coalesced a precondition and remove the explicit calls.
  internal::values::sort(&left);
  coalesce(&left);
  coalesce(&left, right);

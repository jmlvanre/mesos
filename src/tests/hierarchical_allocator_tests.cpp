...
protected:
                 ...
                 inverseOfferCallback = None())
  {
    ...
    if (offerCallback.isNone()) {
      offerCallback =
        lambda::bind(&putOffer, &offerQueue, lambda::_1, lambda::_2);
    }
...
private:
  ...
  static void putInverseOffer(
      process::Queue<Revocation>* queue,
      const FrameworkID& frameworkId,
      const hashmap<SlaveID, UnavailableResources>& resources)
  {
    Revocation revocation;
    revocation.frameworkId = frameworkId;
    revocation.resources = resources;

    queue->put(revocation);
  }
...
protected:
  ...
  process::Queue<Allocation> offerQueue;
  process::Queue<Revocation> inverseOfferQueue;
...
TEST_F(HierarchicalAllocatorTest, UnreservedDRF)
{
  ...
  Future<Allocation> allocation = offerQueue.get();

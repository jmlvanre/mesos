...
namespace mesos {
  extern Value::Ranges coalesce(const Value::Ranges& ranges);
  extern Value::Ranges coalesce(
      const Value::Ranges& ranges,
      const Value::Range& range);
} // namespace mesos {
...
TEST(ValuesTest, AddRangeCoalesce)
{
  ...
  ranges = mesos::coalesce(ranges, range2);

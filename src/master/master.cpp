...
void Master::initialize()
{
  ...
  route("/maintenance.start",
        Http::MAINTENANCE_START_HELP,
        [http](const process::http::Request& request) {
          Http::log(request);
          return http.maintenanceStart(request);
        });
  route("/maintenance.stop",
        Http::MAINTENANCE_STOP_HELP,
        [http](const process::http::Request& request) {
          Http::log(request);
...
void Master::finalize()
{
  ...
  foreachvalue (Slave* slave, slaves.registered) {
    ...
    foreach (InverseOffer* inverseOffer, utils::copy(slave->inverseOffers)) {
      // We don't need to update the allocator because the slave has
    ...
    const Resources& oversubscribedResources)
{
  ...
  // We don't need to rescind inverse offers here as they are
  // unrelated to oversubscription.
...
Nothing Master::updateUnavailability(
    const MachineInfo& machineInfo,
    const Option<Unavailability>& unavailability)
{
  if (unavailability.isSome()) {
    maintenanceStatuses[machineInfo].unavailability = unavailability.get();
  } else {
    maintenanceStatuses.erase(machineInfo);
  }
  ...
  if (machineInfos.contains(machineInfo)) {
    ...
    foreach (const SlaveID& slaveId, machineInfos[machineInfo]) {
      // The slave should not be in the machines mapping if it is
      // removed.
      CHECK(slaves.removed.get(slaveId).isNone());

      // The slave should be registered if it is in the machines
      // mapping.
      ...
      if (unavailability.isSome()) {
        ...
        Try<Time> start = Time::create(unavailability.get().start());
        CHECK_SOME(start);

        LOG(INFO) << "Updating unavailability of slave " << *slave
                  << ", starting at " << start.get();
      } else {
        LOG(INFO) << "Removing unavailability of slave " << *slave;
      }

      slave->unavailability = unavailability;

      // Remove and rescind offers since we want to inform frameworks
      // of the unavailability change as soon as possible.
      foreach (Offer* offer, utils::copy(slave->offers)) {
        allocator->recoverResources(
            offer->framework_id(), slave->id, offer->resources(), None());
      ...
      // Remove and rescind inverse offers since the allocator will
      // send new inverse offers for the updated unavailability.
      foreach (InverseOffer* inverseOffer, utils::copy(slave->inverseOffers)) {
        allocator->updateInverseOffer(
            slave->id,
            inverseOffer->framework_id(),
      ...
      // We remove / resind all the offers first so that any calls to
      // the allocator to modify its internal state are queued before
      // the update of the unavailability in the allocator. We do this
      // so that the allocator's state can start from a "clean slate"
      // for the new unavailability.
      // Note: Any calls from the Allocator back into the master, for
      // example `offer()`, are guaranteed to happen after this
      // function exits due to the Actor pattern.
...
void Master::inverseOffer(const FrameworkID& frameworkId,
                   const hashmap<SlaveID, UnavailableResources>& resources)
{
  ...
  foreachpair (const SlaveID& slaveId,
    ...
    // This could happen if the allocator dispatched
    // 'Master::inverseOffer' before the slave was deactivated in the
    // allocator.
    if (!slave->active) {
      LOG(INFO)
    ...
    // have unique ids accross both. This way we can re-use some of
    // the `OfferID` only messages.
    ...
    Option<Counter> reason)
{
  ...
  foreach (InverseOffer* inverseOffer, utils::copy(slave->inverseOffers)) {
    // We don't need to update the allocator because we've already
    // callled `RemoveSlave()`.
    // Remove and rescind inverse offers.
    removeInverseOffer(inverseOffer, true); // Rescind!
  }
...
void Master::removeInverseOffer(InverseOffer* inverseOffer, bool rescind)
{
  ...
  // Remove and cancel inverse offer removal timers. Canceling the
  // Timers is only done to avoid having too many active Timers in
  // libprocess.
